...
@Component({
  selector: 'app-ventas-contingencia',
  templateUrl: './ventas-contingencia.component.html',
  styleUrls: ['./ventas-contingencia.component.scss']
})
export class VentasContingenciaComponent implements OnInit {
  ...
  constructor(private ventasService: VentasService,
    ...
    onDateSelection(date: NgbDate) {
      ...
      if (!this.fromDate && !this.toDate) {
        ...
        this.paginar(1, this.buscarCliente, this.buscarEstado, this.buscarNroF, this.limit, this.fechaInicio.toString(), this.fechaFin.toString(), this.buscarTipoEmision)
        this.input.close();
      } else {
        this.toDate = null;
        this.fromDate = date;
    ...
    actualizarDatos() {
      ...
      this.fechaInicio = "";
      this.fechaFin = "";
      this.buscarTipoEmision = "2";
      this.model = "";
      this.close();
      this.paginar(1, this.buscarCliente, this.buscarEstado, this.buscarNroF, this.limit, this.fechaInicio, this.fechaFin, this.buscarTipoEmision);
    }

    paginar(page: number, buscarC, buscarE, buscarF, limit, desde, hasta, tipoE) {
      ...
      this.aVentas = [];
      this.ventasService.getVentas(page, '', buscarC, buscarE, buscarF, limit, desde, hasta, tipoE, '').then(data => {
      if (data.finalizado) {
        this.aVentas = data.datos;
        console.log('VENTAS CONTINGENCIA******');
        console.log(this.aVentas);
        this.filtrarAVentas(this.aVentas);
  ...
  buscar() {
    if (this.buscarCliente != "" || this.buscarNroF != 0 || this.buscarEstado != "" || this.model || this.buscarTipoEmision) {
      this.detalle(true);
    }
  }
  ...
  enviarPaquete(){
    this.contingenciaService.postPaquete().then(data =>{
      if (data.finalizado) {
